...
LANG_MAP = {
    ...
    # (한국어 and English details omitted for brevity, but retained in function)
}
# Add Korean and English back in to LANG_MAP...
LANG_MAP["한국어"] = {
    "title": "【휘인 소울 시각 테스트】",
...
def trigger_sticker_animation(sticker_filename):
    ...
    for i in range(20):
        ...
        sticker_html += f"""
        <div class="custom-sticker" style="
            left: {left}%; 
            animation-delay: {delay}s; 
            transform: scale({scale}) rotate({rotate}deg);
            background-image: url('data:image/png;base64,{sticker_base64}');
        "></div>
        """
    ...
    st.markdown(f"""
        ...
        .custom-sticker {{
            ...
            animation: sticker-fall 4s linear infinite; /* 飄落時間 4s */
        }}
        </style>
...
# C. 結果顯示畫面
    ...
    # 【關鍵修改】刪除標準 st.balloons()，換成客製化貼紙動畫！
